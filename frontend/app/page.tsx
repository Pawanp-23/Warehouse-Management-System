...
export default function WmsApp() {
  ...
  async function postSetup(path: string, body: object, label: string) {
    try {
      ...
      showSuccess(`${label} created · UUID ${created.id}`);
      await refresh();
      return created;
    } catch (error) {
      showError(error);
...
function AutomationDirectory({
  ...
  return (
    <DataTable
      title="Automation UUID directory"
      subtitle="Human-readable master data mapped to stable API identifiers. Copy these values for imports, agents, scanners, and external workflows."
    >
          ...
          <form className="form-grid" onSubmit={submit}>
            <Select
              ...
              options={orders
                ...
                .map((item) => [item.id, item.order_number])}
            />
            <Input
              name="carrier_id"
              label="Carrier ID"
        ...
        >
          <form className="form-grid" onSubmit={submit}>
            <Select
              ...
              value={orderId}
              onChange={setOrderId}
              options={orders.map((item) => [item.id, item.order_number])}
            />
            <Input
              name="subtotal"
              label="Subtotal (USD)"
...
function Select({
  ...
  return (
    <label className="field">
      ...
      {showMachineId && value && <MachineId value={value} />}
    </label>
  );
}
